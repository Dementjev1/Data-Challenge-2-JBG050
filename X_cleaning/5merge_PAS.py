...
path = 'Data/twitter_data_pop.csv'       # Path to data
...
# Name columns
derived_df = derived_df.rename(columns={'entries': 'count_Twitter', 'image': 'p_type_image', 'text': 'p_type_text',
                                        'video': 'p_type_video', 'Population': 'population', 'count': 'count_PAS'})

# Order columns
derived_df = derived_df[['borough', 'year', 'population', 'gender', 'race', 'like', 'retweet', 'reply', 'post_length', 'p_type_image',
                         'p_type_text', 'p_type_video', 'count_Twitter', 'mean_Q131', 'mean_Q133', 'Q131_Q133_avg',
                         'count_PAS', 'total_surveys_borough', 'proportion_subgroup']]

print(tabulate(derived_df, headers='keys'))

# Save dataframe to csv file
derived_df.to_csv('Data/data_merged.csv', encoding='utf', index=False)
